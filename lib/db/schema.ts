...
export const notificationChannels = pgTable('notification_channels', {
  id: text('id').primaryKey(), groupId: text('group_id').notNull().references(() => supportGroups.id, { onUpdate: 'cascade', onDelete: 'cascade' }), name: text('name').notNull(), type: channelType('type').notNull(), status: channelStatus('status').notNull().default('ACTIVE'), encryptedConfig: text('encrypted_config').notNull(), configNonce: text('config_nonce').notNull(), configAuthTag: text('config_auth_tag').notNull(), keyVersion: integer('key_version').notNull(), recipientDisplay: text('recipient_display'), redactedDestination: text('redacted_destination'), includeReporterContext: boolean('include_reporter_context').notNull().default(false), lastSucceededAt: utcTimestamp('last_succeeded_at'), lastFailedAt: utcTimestamp('last_failed_at'), createdAt: utcTimestamp('created_at').notNull(), updatedAt: utcTimestamp('updated_at').notNull(),
