...
export default async function ApplicationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!app) notFound()
  return <AppShell><header className="topbar"><div><p className="eyebrow">Application</p><h1>{app.name}</h1><p className="subtle">Ownership, connector status, and current notification policy.</p></div><Badge tone={app.status === 'PAUSED' ? 'warning' : app.enrollmentStatus === 'ACTIVE' ? 'success' : 'neutral'}>{app.status === 'PAUSED' ? 'Suspended' : label(app.enrollmentStatus)}</Badge></header>
    <ApplicationLifecycle app={app} action={applicationLifecycleAction} />
