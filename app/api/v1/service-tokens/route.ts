...
import { recordAssertionReplay, verifyClientAssertion } from '@/lib/service-auth/assertions'
import { getActiveCredential } from '@/lib/service-auth/credentials'
import { publicError, readJsonBody, requestCorrelationId, RequestBodyError } from '@/lib/service-auth/guards'
...
export async function exchangeClientAssertion(assertion: string): Promise<string> {
  return getDb().transaction(async (tx) => {
    ...
    try {
      return await issueServiceAccessToken({ principal, expiresInSeconds: 300 })
    } catch {
...
export async function handleServiceToken(request: Request, deps: { exchangeAssertion?: typeof exchangeClientAssertion } = {}): Promise<Response> {
  ...
  try {
    ...
    const accessToken = await (deps.exchangeAssertion ?? exchangeClientAssertion)(body.clientAssertion)
    return Response.json({ accessToken, tokenType: 'Bearer', expiresIn: 300 })
  } catch (error) {
