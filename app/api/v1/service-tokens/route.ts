...
export async function exchangeClientAssertion(assertion: string, audience: string): Promise<{ accessToken: string; scopes: ServiceScope[] }> {
  return getDb().transaction(async (tx) => {
    const principal = await verifyClientAssertion({
      db: tx as never, assertion, audience,
...
export async function handleServiceToken(request: Request, deps: {
  exchangeAssertion?: typeof exchangeClientAssertion
  publicOrigin?: string
} = {}): Promise<Response> {
  ...
  try {
    ...
    const { audience } = serviceEndpointMetadata(deps.publicOrigin)
    const result = await (deps.exchangeAssertion ?? exchangeClientAssertion)(body.clientAssertion, audience)
