...
export default function InvitationReveal({ appId, invitationId, invitationSecret, expiresAt }: { appId: string; invitationId: string; invitationSecret: string; expiresAt: string }) {
  ...
  useEffect(() => {
    ...
    const guardedState = { ...(isHistoryState(originalState) ? originalState : {}), [HISTORY_GUARD_KEY]: invitationId }
    window.history.replaceState(guardedState, '', revealUrl)
    const protectBack = (event: PopStateEvent) => {
      event.stopImmediatePropagation()
      if (isInvitationGuardState(event.state, invitationId)) return
      setLeaveWarning(true)
      window.history.forward()
    }
    window.addEventListener('beforeunload', protectUnload)
    window.addEventListener('popstate', protectBack, true)
    document.addEventListener('click', protectLink, true)
    return () => {
      window.removeEventListener('beforeunload', protectUnload)
      window.removeEventListener('popstate', protectBack, true)
      document.removeEventListener('click', protectLink, true)
      if (isInvitationGuardState(window.history.state, invitationId)) window.history.replaceState(originalState, '', revealUrl)
    }
...
function isInvitationGuardState(value: unknown, invitationId: string): boolean {
  return isHistoryState(value) && value[HISTORY_GUARD_KEY] === invitationId
}
